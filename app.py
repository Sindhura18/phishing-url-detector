...
with tab_url:
    ...
    with col1:
        if st.button("🟢 Legitimate (Google Search)", key="btn_legit"):
            st.session_state.url_scanner_input = "https://www.google.com/search?q=python+tutorial"
            st.rerun()
    with col2:
        if st.button("🔴 Critical Threat (IP-based Phish)", key="btn_phish"):
            st.session_state.url_scanner_input = "http://192.168.1.1/login@paypal-secure-verify.com/account/update"
            st.rerun()
    with col3:
        if st.button("🟡 High Risk (New/Untrusted Domain)", key="btn_risk"):
            st.session_state.url_scanner_input = "http://secure-netflix-update-billing.com/signin.html"
            st.rerun()
    ...
    if current_url:
        ...
        with st.spinner("Executing real-time DNS checks, WHOIS lookup, and AI model predictions..."):
            try:
                ...
                if response.status_code != 200:
                    st.error(f"Error scanning URL: {response.json().get('detail', 'Unknown error')}")
                    st.stop()
                    
                result = response.json()
            except Exception as e:
                st.error(f"Could not connect to the scanner service: {e}")
                st.stop()
...
with tab_email:
    ...
    # Template loader
    if st.button("📋 Load Spoofed Email Template"):
        st.session_state.email_text_input_area = (
    ...
    trigger_email_scan = False
    if email_text_val.strip():
        if st.button("Analyze Email Message", type="primary", key="btn_scan_email"):
            trigger_email_scan = True
            st.session_state.email_input_text = email_text_val # Keep synced
            
            with st.spinner("Analyzing email headers, validating sender signatures, and scanning links..."):
                try:
                    headers = {"X-API-Key": API_KEY}
                    payload = {"email_text": email_text_val}
                    ...
                    st.error(f"Could not connect to the security API: {e}")
                    st.stop()
            
            # Save results in session state so they persist on reruns
        ...
        if "last_email_result" in st.session_state:
